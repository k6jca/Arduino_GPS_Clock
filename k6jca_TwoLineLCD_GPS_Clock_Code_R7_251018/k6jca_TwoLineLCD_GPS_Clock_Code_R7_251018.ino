// k6jca_TwoLineLCD_GPS_Clock_Code_Rev_251018.ino
// 18 October 2025
//
// based on k6jca_LCD_GPS_Clock_Code_Rev_251013.ino
//
// This code supports both 2x16 character and 2x20 character
// displays.  But it needs to be recompiled based upon the 
// number of characters in a line (16 v. 20).
//
// To do this, set the LCD_COLS value to either 20 or 16 
// and recompile.
// 
// Possible mods for the future...
//   1.  Use state of D4 to select 20 or 16 characters per line
//   2.  Use state of D5 to select 0x3F or 0x27 as the 
//       display's I2C address
//
//
// k6jca 
...
// ---------------- Menu system ----------------
// Menus are:  0: Day of Week
//             1: Date
//             2: Satellites
//             3: Local time offset
//             4: Display Seconds (on/off)
//             5: GPS Baud Rate (4800 - 38400)
//             6: No Backlight if Dark (on/off)
//             7: Ambient Light Level
//             8: GMT Time
//             9: Select Power-on Menu
//
const int numMenuItems = 10;  // Number of menus
//int currentMenuItem = 1;     // init to satellites menu
int currentMenuItem = 0;     // init to day-of-week on menu line
int lastMenuItem = currentMenuItem;
int pwrOnMenuItem = currentMenuItem;

const int dayMenu = 0;
const int dateMenu = 1;
const int satsMenu = 2;
const int GMTMenu = 8;
...
const int EEPROM_ADDR_BACKLIGHT = 7;
const int EEPROM_ADDR_PWRON_MENU = 8;
...
void setup() {
  ...
  noBkliteIfDark = EEPROM.read(EEPROM_ADDR_BACKLIGHT) != 0;
  ...
  int storedMenuItem = EEPROM.read(EEPROM_ADDR_PWRON_MENU);
  if (storedMenuItem < 0 || storedMenuItem > (numMenuItems-1)) currentMenuItem = 0;
  else currentMenuItem = storedMenuItem;
  pwrOnMenuItem = currentMenuItem;
  Serial.print(F("Stored Power-On Menu =  "));
  ...
  for (int i = 0; i < numBauds; i++) {
    if (detectBaud(supportedBauds[i])) {
      ...
      if(LCD_COLS == 20) {
        ...
        if (currentBaud < 10000) lcd.setCursor(16,1);
        else lcd.setCursor(15,1);
      }
      else {
        lcd.print(F("Detected:       "));
        if (currentBaud < 10000) lcd.setCursor(12,1);
  ...
  while(test == true) { // *** For testing.  At end of loop set test to false for 1 pass thru
    ...
    if (now - lastSampleTime >= debounceInterval) {
      ...
      if (lastMenuState == LOW && currentMenuState == HIGH) {  // step to next menu
        ...
        if (lastMenuItem == 4) {  // display seconds
        ...
        if (lastMenuItem == 6) {
          EEPROM.update(EEPROM_ADDR_BACKLIGHT, noBkliteIfDark);
          Serial.println(F("  No Backlight if Dark EEPROM updated"));        
        }
        if (lastMenuItem == 9) {
          EEPROM.update(EEPROM_ADDR_PWRON_MENU, pwrOnMenuItem);
          Serial.println(F(" Power-On Menu Selection Updated"));
        }
      ...
      // Note that menu items for 0 (day of week) and 1  (satellites) have no actions -- they simply
      ...
      if (lastNextState == LOW && currentNextState == HIGH) {
        switch (currentMenuItem) {
          ...
          case 6:  // No backlight if dark
            ...
            case 9:  // power-on menu selection
              switch (pwrOnMenuItem) {
                case dayMenu: pwrOnMenuItem = dateMenu; break;
                case dateMenu: pwrOnMenuItem = satsMenu; break;
                case satsMenu: pwrOnMenuItem = GMTMenu; break;
                case GMTMenu:  pwrOnMenuItem = dayMenu; break;
                default: pwrOnMenuItem = dayMenu; break;
              }
...
bool detectBaud(long baud) {
  ...
  if(LCD_COLS == 20) {
    ...
    if (baud < 10000) lcd.setCursor(16,1);
    else lcd.setCursor(15,1);  
  }
  else {
    lcd.print(F("Try baud:       ")); 
    if (baud < 10000) lcd.setCursor(12,1);
    else lcd.setCursor(11,1);  
  }
  lcd.print(baud);
...
void drawMenuLine() {
  ...
  // 2nd line: Menu selection and menu item within that menu
  lcd.setCursor(0,1);
  switch (currentMenuItem) {
    case 0:  // display day of week
      if (!gps.time.isValid() || !gps.date.isValid() || !receivingTime) {   // 251028 add this because sometime date is wrong
        // Show if waiting or invalid
        if(LCD_COLS == 20) lcd.print(F("  "));
        lcd.print(F("     ------     "));
        if(LCD_COLS == 20) lcd.print(F("  "));
        //lcd.print(F("     --/--/----     "));
        ...
        // Day of week line:
        dow = dayOfWeek(localDay, localMonth, localYear); // 0..6
    ...
    case 1:  // display date
      if (!gps.time.isValid() || !gps.date.isValid() || !receivingTime) {  // 251028
        // Show if waiting or invalid
        if(LCD_COLS == 20) lcd.print(F("  "));
        lcd.print(F("     ------     "));
        if(LCD_COLS == 20) lcd.print(F("  "));
    ...
    case 4:  // display seconds
      if(LCD_COLS == 20) lcd.print(F("Display Seconds: "));
      else lcd.print(F("Inc Seconds: "));
    ...
    case 5:   // baud rate
      if(LCD_COLS == 20) {
        ...
        if (desiredBaud < 10000) lcd.setCursor(16,1);
        else lcd.setCursor(15,1);
      }
      else {
        lcd.print(F("GPS Baud:    "));
        if (desiredBaud < 10000) lcd.setCursor(12,1);
        else lcd.setCursor(11,1);
      }
      lcd.print(desiredBaud);
      break;

    case 6:  // display seconds
      if(LCD_COLS == 20) lcd.print(F("BkLt when Dark:  "));
      else lcd.print(F("BkLt @ dark: "));
      lcd.print(noBkliteIfDark ? "OFF" : "ON ");
      break;
    ...
    case 7: 
      if(LCD_COLS == 20) {
        ...
        lcd.print(F("Light Lvl:      "));
        lcd.setCursor(12,1);
      }
      lcd.print(rawAnalog);
      break;

    case 8: // display GMT time
      ...
      //} else {
      //  if (colonState == true) {
          if(LCD_COLS == 20) t = "        " +twoDigits(gpsHour) + ":" + twoDigits(gpsMinute) ;
          else t = "      " +twoDigits(gpsHour) + ":" + twoDigits(gpsMinute) ;
          lcd.print(t);
          if(LCD_COLS == 20) lcd.print(F("    GMT")); 
          else lcd.print(F("  GMT")); 
      //}
      break;

      case 9:  // select menu to be displayed at powerup
        if(LCD_COLS == 20) {
          lcd.print(F("Power-On Menu = "));

        } else {
          lcd.print(F("PwrOn Menu= "));
        }
        switch(pwrOnMenuItem) {
          case dayMenu: lcd.print(F("Day ")); break;
          case dateMenu: lcd.print(F("Date")); break;
          case satsMenu: lcd.print(F("Sats")); break;
          case GMTMenu: lcd.print(F("GMT ")); break;
          default: lcd.print(F(" ERR")); break;
        }
      break;
...
void drawTimeDateDay() {
  ...
  if (!autoDST) {  // Not in auto-DST mode.  Format time depending upon whether GMT or not.
    ...
    } else {  // check if DST or not.  Only display time in 12-hour format
...
/*
  if(currentMenuItem == 1) { // print date in menu line
    // Print Date
    lcd.setCursor(0,1);
    lcd.print(F(" ")); // move over one space 
...
/*
  if(currentMenuItem == 0) {// print day of week in menu line
    // Day of week line:
  ...
  if (currentBaud != desiredBaud) {
    ...
    while (!success) {
      ...
      if (detectBaud(desiredBaud)) {
        ...
        if(LCD_COLS == 20) lcd.print(F("  Baud Change OK!   "));
        else lcd.print(F("Baud Change OK! "));
        ...
        if (attempt > 4) {
          ...
          // show error   
          lcd.setCursor(0,1);
          if(LCD_COLS == 20) lcd.print(F("ERR: Did Not Verify "));
          else lcd.print(F("ERR: VERIFY     "));
          delay(3000);
          break;
        }
      }      
    ...
    if (!success) {
      Serial.println(F("❌ Could not change GPS baud."));
        ...
        // show error   
        lcd.setCursor(0,1);
        if(LCD_COLS == 20) lcd.print(F("FAIL TO CHANGE BAUD "));
        else lcd.print(F("FAIL BAUD CHANGE"));
        delay(3000);
      }
  } else {
